...
test('extension UI and interceptor use SillyTavern third-party-compatible registration', () => {
    ...
    assert.match(source, /ignoredPromptKeys = new Set\(\[PROMPT_KEY, 'continuity_memory_context'\]\)/);
    ...
    assert.match(source, /injectionPosition: 'at-depth', injectionDepth: 3, injectionRole: 'user'/);
    assert.match(source, /context\.chat\?\.length \? context\.chat : chat/);
    assert.match(source, /Always-guide means the outgoing roleplay must not outrun its planner/);
    ...
    assert.match(template, /data-setting="injection-depth"[^>]*value="3"/);
