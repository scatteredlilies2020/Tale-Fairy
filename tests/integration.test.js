...
test('manifest, browser runtime, and detached plugin share the release version', () => {
    assert.equal(manifest.version, '0.14.35');
    assert.equal(manifest.js, 'extension/index.js?v=0.14.35');
    assert.equal(manifest.css, 'extension/style.css?v=0.14.35');
    assert.equal(pluginPackage.version, manifest.version);
    assert.match(pluginSource, /const VERSION = '0\.14\.35'/);
    assert.match(source, /const RUNTIME_VERSION = '0\.14\.35'/);
});
