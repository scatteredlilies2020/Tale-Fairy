...
test('manifest, browser runtime, and detached plugin share the release version', () => {
    ...
    assert.equal(manifest.js, 'extension/index.js?v=0.14.9');
    assert.equal(manifest.css, 'extension/style.css?v=0.14.9');
    assert.equal(pluginPackage.version, manifest.version);
    assert.match(pluginSource, /const VERSION = '0\.14\.9'/);
    assert.match(source, /const RUNTIME_VERSION = '0\.14\.9'/);
});
...
test('runtime uses causal context and deferred world state without prescriptive beat-director dependency', () => {
    assert.match(source, /from '\.\/causal-context\.js\?v=0\.14\.9'/);
    assert.match(stateSource, /from '\.\/causal-context\.js\?v=0\.14\.9'/);
