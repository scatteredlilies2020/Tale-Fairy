...
test('manifest and detached plugin identify the adaptive-director release', () => {
    assert.equal(manifest.version, '0.12.8');
    assert.equal(manifest.js, 'extension/index.js?v=0.12.8');
    assert.equal(manifest.css, 'extension/style.css?v=0.12.8');
    assert.match(manifest.description, /always-on adaptive story director/i);
    assert.equal(pluginPackage.version, manifest.version);
    assert.match(pluginSource, /const VERSION = '0\.12\.8'/);
    assert.match(source, /const RUNTIME_VERSION = '0\.12\.8'/);
});

test('extension loads the background-only direction modules', () => {
    assert.match(source, /from '\.\/analysis\.js\?v=0\.12\.8'/);
    assert.match(source, /from '\.\/state\.js\?v=0\.12\.8'/);
    assert.match(source, /from '\.\/request-injection\.js\?v=0\.12\.8'/);
    assert.match(source, /from '\.\/director-sampling\.js\?v=0\.12\.8'/);
    assert.doesNotMatch(source, /action-gate/);
    assert.match(stateSource, /from '\.\/beat-director\.js\?v=0\.12\.8'/);
});
