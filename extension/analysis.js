...
const DISTANT_HORIZON_PATTERN = /\b(?:later[- ]arcs?|future[- ]arcs?|several[- ]arcs?|multiple[- ]arcs?|multi[- ]arc|distant[- ](?:arc|phase|time|future)|long[- ]term|long[- ]range|months?|years?|decades?|lifetime|open[- ]ended)\b/iu;
...
export function validateAnalysisResult(result) {
    ...
    if (!horizons || typeof horizons !== 'object' || Array.isArray(horizons)) {
        ...
        if (farthest && !DISTANT_HORIZON_PATTERN.test(String(farthest.timeframe || ''))) errors.push('the highest plan horizon must name a later arc, distant time, or another open-ended long timeframe');
