...
export const ANALYSIS_SCHEMA_VALUE = {
    ...
    properties: {
        ...
        cue_audit: { type: 'object', additionalProperties: false, properties: {
            offered_ids: { type: 'array', maxItems: 3, items: { type: 'string', maxLength: 100 } }, manifested_ids: { type: 'array', maxItems: 3, items: { type: 'string', maxLength: 100 } }, unused_ids: { type: 'array', maxItems: 3, items: { type: 'string', maxLength: 100 } }, contradicted_ids: { type: 'array', maxItems: 3, items: { type: 'string', maxLength: 100 } }, pacing: { type: 'string', enum: ['respected','exceeded','uncertain'] }, reason: { type: 'string', maxLength: 300 },
...
After juggling the scene, world model, possibilities, pathways, objectives, schedules, and every time horizon privately, return two or three ranked next_guides as a compact conditional background cue set. The first is the strongest ready possibility to emphasize; all remain optional, and the roleplay model may use zero or one after applying use_when and drop_when to the newest user turn. The candidates must differ in actual development or causal source—not merely wording—and their world_delta values must describe genuinely distinct possible visible changes while preserving established facts, characterization, causal continuity, pacing, and player agency. Contrast never licenses randomness or contradiction. Every candidate must have a specific direction, use_when, drop_when, optional response_bias, exactly one possible world_delta, an origin classification, concise factual basis, lean strength, source pathway ids, and reason. Keep each field concise. The world_delta is a possible effect within or immediately at the endpoint of the player's declared action; it must not consume a later implied activity. Write use_when and drop_when as binding in-story conditions. Never mention swipes, generation metadata, writing, or candidate rank inside a cue. A cue may propose an NPC move, consequence, discovery, social turn, world development, or fresh continuation, but it is not a prediction, commitment, required outcome, or fact that already happened. A transition-only candidate is valid when it is the complete user-authorized temporal scope.
