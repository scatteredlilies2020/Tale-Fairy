import { fingerprintMessages, normalizeState, stateForPrompt } from './state.js?v=0.12.4';
...
import { formatDirectorSample, sampleDirectorSignals } from './director-sampling.js?v=0.12.4';
...
export function buildAnalysisPrompt(messages, state, note = '', bootstrap = {}, options = {}) {
    ...
    const payload = {
        ...
        contribution_rule: 'Always set beat.inject=true for a fresh analysis. Avoid interrogation: do not keep NPCs asking the user to explain, justify, clarify, or answer repeated follow-up questions after the user has responded. Prefer NPC decisions, reactions, actions, consequences, or natural forward movement; ask only a genuinely necessary question or one explicitly requested by the user. Add only a complementary, context-native NPC or world response, consequence, opportunity, or next causal step. If a branch would interpret or compete with the user action, the writing model must ignore it. Quiet listening, assignments, rest, travel, and other routine activity still receive something external to notice, exchange, discover, advance, or respond to. Keep it subtle when warranted; never manufacture conflict, interruption, pressure, urgency, or restriction merely to make something happen. Explain the private choice in inject_reason.',
