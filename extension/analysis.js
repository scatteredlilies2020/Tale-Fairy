...
export function validateAnalysisResult(result) {
    ...
    for (const [index, guide] of (Array.isArray(result.next_guides) ? result.next_guides : []).entries()) {
        ...
        if (!guide?.id?.trim() || !guide?.direction?.trim() || !guide?.use_when?.trim() || !guide?.drop_when?.trim() || !guide?.causal_role?.trim() || !guide?.world_delta?.trim() || !guide?.basis?.trim()) errors.push(`next_guides[${index}] must be a grounded conditional movement with a causal role and meaningful aftereffect`);
    ...
    if (new Set(guideIds).size !== guideIds.length || new Set(guideDirections).size !== guideDirections.length || new Set(guideDeltas).size !== guideDeltas.length) {
        errors.push('next_guides must use distinct ids, contrasting directions, and distinct visible changes');
    }
...
export function repairAnalysisResult(result) {
    ...
    if (!repaired.pathways.length && repaired.next_guides.length) {
        ...
        repaired.pathways = [{ id: 'recovered-path-1', direction: guide.direction, when: guide.use_when, response_bias: guide.causal_role, horizon: 'next few turns', status: 'foreground', conditions: [], change: 'adjust', reason: guide.basis }];
...
const PROMPT_PACING_INSTRUCTION = 'USER-CONTROLLED PACING — Match the user’s demonstrated speed and granularity. Treat the latest user action as the maximum authorized player-action progress, not permission to complete later implied activities. Complete exactly what was declared without adding permission checkpoints: heading, going, or walking to a destination permits travel and arrival only, not doing or finishing the activity there, advancing to the next task, or skipping additional time. An NPC or world development may occur during that same action or upon arrival, but it must not require crossing the action boundary. Slow pacing means meaningful development inside the current beat, not artificial delay; mode changes narrative pressure, not speed. Only explicit requests to advance, skip, continue until, or reach a milestone authorize broader progress. If established facts make an action impossible, show the attempt and concrete obstacle. Let causal motives, constraints, hidden information, and world processes determine interesting outcomes without inventing the player’s choices. Respond directly to the complete latest user turn.';
...
current.lastOfferedCues, when present, records optional conditional background cues sent with the newest assistant request. requestConfirmed proves only that the cue set was present; no cue was promised or required. Audit each offered id against the newest assistant prose: manifested only when its possible effect actually became true, contradicted only when the prose establishes an incompatible fact, and unused otherwise—including when its IF was false, its UNLESS was true, or the response simply followed another valid course. An unused cue creates no failure, repair debt, repetition pressure, or obligation to offer it again. Return this classification in cue_audit and independently mark pacing exceeded only when the assistant went beyond the newest user's authorized action boundary. Do not turn an audit into prose-style criticism.
...
Give each next guide one coherent causal movement and one meaningful possible aftereffect. Do not dictate a sequence of concrete events, a mandatory reveal, or an exact character action merely to achieve it. Keep direction about the story-state transition and world_delta about what may meaningfully shift by its end. The roleplay model must retain room to judge the concrete realization from the complete current scene.
...
Maintain narrative momentum inside the user's action boundary. A routine transition may be the entire temporal scope when it is what the user declared. Enrich it with a supported NPC initiative, reaction, information change, relationship movement, discovery, opportunity, cost, payoff, complication, external process, or meaningful resolution only when that development can occur during the action or at its endpoint. Never finish a subsequent activity or jump to a later task merely to manufacture a stronger delta. Arrival, an encounter during transit, or a changed situation at the destination is a valid world_delta when further progress would rush the user's timeline. Protecting pacing never requires NPC passivity.

A world_delta is the desired narrative aftereffect, not a demanded plot event. It must matter after the response by changing knowledge, stakes, relationships, available choices, resources, obligations, emotional orientation, or the direction of a live process. A routine notice, harmless ping, decorative interruption, minor physical symptom, or atmospheric detail is not sufficient merely because it is new. Originality means a fresh causal development, not low-stakes procedural noise used to avoid a stronger supported thread. Prefer a live consequence or recently charged unresolved thread when it can move now; allow the roleplay model to invent the exact mechanism or surprise when established characterization and world causality support it.
...
The causal control and first next guide must agree, but they serve different purposes: causal control preserves overall direction and rate of state change; the concrete guide offers one optional current realization. Rank first a movement whose IF is true and UNLESS is false at analysis time and which remains executable by an NPC or world process under the broadest reasonable continuation of the current scene. This ranking only chooses which optional movement is offered; it does not make that movement a foreground requirement. Never rank a departed, absent, completed, superseded, or merely historical actor/process first. Put narrower alternatives later. If every specific development is fragile, use a broadly applicable but meaningful causal step rather than a likely no-op.
...
Use direction for a high-level causal transition rather than a screenplay. causal_role states whether and how the movement holds, seeds, advances, converges, pays off, redirects, or recovers a specific live thread. It must describe cause-and-effect function only and must never mention mood, emotional tone, prose tempo, dialogue delivery, focus, surprise, sentence form, formatting, verbosity, or stylistic imitation. Make world_delta the possible meaningful aftereffect while leaving the concrete mechanism to the roleplay model. Preserve established facts, causal continuity, pacing, information boundaries, and player agency. The aftereffect must occur within or immediately at the endpoint of the player's declared action; it must not consume a later implied activity. Write use_when and drop_when as binding in-story conditions. Never mention swipes, generation metadata, writing, or candidate rank inside a movement.
...
Pathways, possibilities, objectives, entities, schedules, narrative events, future_setup, plan horizons, audit details, and the general guidance field are private planning material and are never copied wholesale into the roleplay prompt. Only the non-secret story identity, current scene function, setting forces, causal tempo, arc direction, meaningful aim, and one optional movement may reach the roleplay request. Always return inject=true, director_score, three to four contrasting conditional causal movements in next_guides, one to five pathways, six to ten plan horizons, cue_audit, and a compact guidance string for the private scratchpad; guidance may be empty. Keep the ledger compact.`;
