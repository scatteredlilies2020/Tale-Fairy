...
export const PACING_INSTRUCTION = 'USER-CONTROLLED PACING — Infer pacing from the user\'s recent actions, level of detail, explicit requests, and willingness to linger or advance. Match that pacing. Slow pacing means giving a meaningful beat room; it never means fragmenting one obvious action across several replies, manufacturing a queue or threshold, repeating the user\'s action without its consequence, or asking permission for something the user already chose. Every declared action, direct question, and choice authorizes its ordinary continuation and immediate meaningful consequence; no pacing keyword is required. Infer and perform routine implied steps such as crossing an already-opened doorway, approaching the stated destination, receiving an available result, or allowing an addressed NPC to answer. Do not make the user micromanage mechanics that introduce no genuinely new consequential choice. Never deliberately speed up, slow down, time-skip, montage, compress, prolong, or resolve the story because of the selected mode. When the user additionally says advance, proceed, continue, until done, to my turn, or now, treat that as even more explicit binding minimum progress: reach the requested milestone in the next reply. Do not instruct the main model to withhold an available answer or consequence, stop just before it, or turn it into another consent checkpoint. If an action is impossible under established facts, show the attempt and concrete in-world obstacle instead of stalling. Advance time or conclude a beat when the user requests it, clearly signals it, or the established immediate action necessarily completes it. The mode changes narrative pressure, boldness, and breadth of possibilities—not narrative speed. NPCs and the world may still act within the current moment, and a strong external development may arrive without taking control of how quickly the user responds or moves onward. Guidance is prepared before the next user turn, so it must remain subordinate to whatever that newer turn does, asks, or requests.';
