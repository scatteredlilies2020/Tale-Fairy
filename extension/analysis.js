...
export function validateAnalysisResult(result) {
    ...
    for (const [index, guide] of (Array.isArray(result.next_guides) ? result.next_guides : []).entries()) {
        ...
        if (!guide?.id?.trim() || !guide?.direction?.trim() || !guide?.use_when?.trim() || !guide?.drop_when?.trim() || !guide?.response_bias?.trim() || !guide?.world_delta?.trim() || !guide?.basis?.trim()) errors.push(`next_guides[${index}] must be a grounded conditional movement with a dramatic score and meaningful aftereffect`);
...
Give each next guide one coherent dramatic movement and one meaningful desired aftereffect. Do not dictate a sequence of concrete events, a mandatory reveal, or an exact character action merely to achieve it. Keep direction about the response's trajectory and world_delta about what has meaningfully shifted by its end. The roleplay model must retain room to judge the concrete realization from the complete current scene.
