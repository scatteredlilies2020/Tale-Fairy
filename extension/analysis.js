...
export const ANALYSIS_SCHEMA_VALUE = {
    ...
    properties: {
        ...
        next_guides: { type: 'array', minItems: 3, maxItems: 4, items: { type: 'object', additionalProperties: false, properties: {
            id: { type: 'string', maxLength: 100 }, direction: { type: 'string', maxLength: 280 }, use_when: { type: 'string', maxLength: 120 }, drop_when: { type: 'string', maxLength: 100 }, response_bias: { type: 'string', minLength: 1, maxLength: 130 }, world_delta: { type: 'string', maxLength: 140 }, origin: { type: 'string', enum: ['established','inferred','original'] }, basis: { type: 'string', maxLength: 100 }, strength: { type: 'string', enum: ['strong','moderate','light'] }, source_pathways: { type: 'array', maxItems: 3, items: { type: 'string', maxLength: 100 } }, causal_event_ids: { type: 'array', maxItems: 2, items: { type: 'string', maxLength: 80 } }, disclosure: { type: 'string', enum: ['none','consequence-only','partial-clue','reveal-cause'] }, reason: { type: 'string', maxLength: 220 },
...
export function validateAnalysisResult(result) {
    ...
    for (const [index, guide] of (Array.isArray(result.next_guides) ? result.next_guides : []).entries()) {
        ...
        if (!guide?.id?.trim() || !guide?.direction?.trim() || !guide?.use_when?.trim() || !guide?.drop_when?.trim() || !guide?.response_bias?.trim() || !guide?.world_delta?.trim() || !guide?.basis?.trim()) errors.push(`next_guides[${index}] must be a grounded conditional guide with characterful delivery and a visible world delta`);
...
A world_delta must matter after its sentence: it changes knowledge, stakes, relationships, available choices, resources, obligations, or the direction of a live process. A routine notice, harmless ping, decorative interruption, minor physical symptom, or atmospheric detail is not sufficient merely because it is new. Originality means a fresh causal development, not low-stakes procedural noise used to avoid a stronger supported thread. Prefer a live consequence or recently charged unresolved thread when it can move now; invent a new administrative or ambient event only when it creates a meaningful choice or consequence.
...
After juggling the scene, world model, possibilities, pathways, objectives, schedules, and every time horizon privately, return three or four ranked next_guides as a compact conditional background cue set. Light mode normally uses three; Balanced and Fun should use four when four genuinely distinct supported routes exist, but never pad the set with duplicates. The first is the strongest ready possibility to emphasize; all remain optional, and the roleplay model may use zero or one after applying use_when and drop_when to the newest user turn. The candidates must differ in emotional and causal shape—not merely wording—and their world_delta values must describe genuinely distinct possible visible changes while preserving established facts, characterization, causal continuity, pacing, and player agency. Across the set, vary initiating actors or systems and favor a useful spread of relationship movement, world/process consequence, discovery, opportunity, complication, or payoff when the context supports it; this is a diversity heuristic, not a mandatory menu. Contrast never licenses randomness or contradiction. Every candidate must have a specific direction, use_when, drop_when, non-empty response_bias, exactly one possible world_delta, an origin classification, concise factual basis, lean strength, source pathway ids, and reason. Make response_bias a compact, characterful delivery channel grounded in an established habit, relationship tension, setting mechanism, meaningful object, institution, or sensory world behavior. It should improve flavor without prescribing prose style, sentence shape, formatting, or imitation. Keep each field concise. The world_delta is a possible effect within or immediately at the endpoint of the player's declared action; it must not consume a later implied activity. Write use_when and drop_when as binding in-story conditions. Never mention swipes, generation metadata, writing, or candidate rank inside a cue. A cue may propose an NPC move, consequence, discovery, social turn, world development, or fresh continuation, but it is not a prediction, commitment, required outcome, or fact that already happened. A transition-only candidate is valid when it is the complete user-authorized temporal scope.
...
Pathways, possibilities, objectives, entities, schedules, narrative events, plan horizons, audit details, and the general guidance field are private planning material and are never copied wholesale into the roleplay prompt. Put any candidate-specific execution detail in that next guide's response_bias. Always return inject=true, three to four contrasting conditional background cues in next_guides, one to five pathways, six to ten plan horizons, cue_audit, and a compact guidance string for the private scratchpad; guidance may be empty. Keep the ledger compact.`;
