...
export function buildAnalysisPrompt(messages, state, note = '', bootstrap = {}, options = {}) {
    ...
    const payload = {
        ...
        motive_rule: 'Privately ask why each notable event, acceleration, delay, silence, refusal, coincidence, invitation, or unusual opportunity is happening. hidden_motives has zero to six ranked explanations, with the strongest evidence-based explanation first when there is a meaningful mystery. actor names the suspected person, institution, faction, force, or system; explanation states the hypothesis; mechanism explains how it would cause the observation; evidence and counterevidence cite compact clues. likelihood distinguishes established, most-likely, likely, possible, wild-card, and contradicted. Keep bold franchise-aware or original possibilities when they genuinely fit the evidence, but never add a novelty-only wild-card just to make the list entertaining. current_relevance says whether a motive is irrelevant, background, supports the current beat, or drives the current beat; even a likely motive is never canon merely because it ranks first. Never add a motive to canon_updates, never force a reveal or payoff, and never expose exact motives to the roleplay provider. Explicit user/OOC facts outrank every inferred motive.',
...
AUTHORITY: Explicit user/OOC/scenario commands and the latest user text outrank this entire plan. The user action is outside Tale Fairy's authority. Do not infer, reinterpret, expand, narrow, relocate, complete, substitute, evaluate, or define the user action, its target, its manner, or the player's intent. The main roleplay instructions and context resolve it. A forced outcome binds that outcome. Never use planning to deny, delay, weaken, cap, or modify the user action. Never invent player movement or inner state; never invent player dialogue, thoughts, feelings, consent, decisions, compliance, retreat, or unrelated extra actions.
...
CONDITIONAL MOVEMENT SET: When the newest message is an assistant response, the user's next text is unknown; prepare a compact policy for possible NPC or world responses. When the newest message is already user text, make the primary branch fit only as an external response to that text. beat.primary_when states when the primary external follow-through fits. beat.alternatives contains exactly two materially distinct redirect-safe branches with natural-language conditions. The conditions distinguish external response routes; they do not interpret the user action. The writing model must ignore all branches if they cross into defining or modifying the user action.
...
PRIVATE HIDDEN-MOTIVE BOARD: Maintain zero to six ranked explanations for the notable why behind the current scene: who benefits, what pressure or incentive is operating, what capability or secret could explain unusual timing, and what alternative explanation would fit if the first one is wrong. This board is deliberately open scratchpad space. Put the strongest evidence-based explanation first when one exists, then plausible alternatives. A wild-card or contradictory foil is optional, and belongs only when it is genuinely unsupported, unlikely, or in tension with the evidence; do not add a novelty-only idea for entertainment. Mark every item with likelihood, evidence, counterevidence, mechanism, current relevance, and disclosure. A motive may mention exact names, lore, secrets, hidden actors, or original inventions here because this board is private. It is never a canon fact, never a scheduled reveal, never a player-action instruction, and never provider-visible. Explicit user/OOC facts outrank inferred motives. A motive can help choose the shape of a current NPC/world response only as a private hypothesis; do not turn an unsupported motive into a new factual cause or reveal it as established.
...
hidden_motives={status,items,audit}; status is none, open, or focused. items has at most 6 ranked private hypotheses, each {id,actor,explanation,likelihood,evidence,counterevidence,mechanism,current_relevance,disclosure,change}. likelihood is established, most-likely, likely, possible, wild-card, or contradicted; current_relevance is none, background, supports-beat, or drives-beat; disclosure is hidden, signaled, or revealed; change is keep, adjust, replace, or retire. This is open scratchpad space: exact scene names, secrets, franchise lore, original inventions, unlikely explanations, and contradictory foils are allowed here, but every item must be labeled and never treated as canon solely because it is listed. Keep the strongest explanation first, then alternatives. Hidden motives never enter provider direction or canon. A strong, scene-supported motive may shape the external beat indirectly; a weak or novelty-only motive may not be smuggled in as fact.
