...
export function buildAnalysisPrompt(messages, state, note = '', bootstrap = {}, options = {}) {
    ...
    const payload = {
        ...
        contribution_rule: 'Always set beat.inject=true for a fresh analysis. Add only a complementary, context-native NPC or world response, consequence, opportunity, or next causal step. If a branch would interpret or compete with the user action, the writing model must ignore it. Quiet listening, assignments, rest, travel, and other routine activity still receive something external to notice, exchange, discover, advance, or respond to. Keep it subtle when warranted; never manufacture conflict, interruption, pressure, urgency, or restriction merely to make something happen. Explain the private choice in inject_reason.',
