...
export const PACING_INSTRUCTION = 'USER-CONTROLLED PACING — Infer pacing from the user\'s recent actions, level of detail, explicit requests, and willingness to linger or advance. Match that pacing. Never deliberately speed up, slow down, time-skip, montage, compress, prolong, or resolve the story because of the selected mode. Advance time or conclude a beat only when the user requests it, clearly signals it, or the established immediate action necessarily completes it. The mode changes narrative pressure, boldness, and breadth of possibilities—not narrative speed. NPCs and the world may still act within the current moment, and a strong external development may arrive without taking control of how quickly the user responds or moves onward.';
