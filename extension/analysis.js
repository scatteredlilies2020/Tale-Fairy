...
export const MODE_INSTRUCTIONS = Object.freeze({
    ...
    balanced: 'BALANCED MODE — Combine continuity with moderate intervention that produces meaningful change. Keep one to three distinct supported possibilities and choose the strongest ready development, not the safest or least-committal continuation. The primary guide should add a visible NPC decision, consequential reaction, information change, relationship movement, opportunity, complication, payoff, or setting process while preserving the current tone and leaving the player response open. Fold routine movement, meals, and scene logistics around that development instead of treating them as the development. Moderate intervention does not mean changing the user\'s narrative speed.',
...
current.lastDelivery, when present, records the one route actually sent with the newest assistant request. requestConfirmed proves prompt delivery only, never story compliance. Compare its worldDelta with the newest assistant prose. If the delta did not become true onscreen and remains causally appropriate after the response, make a direct repair the primary next guide; do not treat setup, travel toward it, or a promise to do it later as completion. If the newest response or user direction made it obsolete, discard it instead of repeating it.
...
Track readiness rather than waiting reflexively. When an explicitly established commitment, accepted request, initiated process, planted setup, or other causal obligation reaches its non-player conditions, promote its pathway to foreground and address it in the primary guide. Do not add conditions such as “the player asks,” “the player is receptive,” or “conversation opens” when an NPC or world process can initiate the development naturally. If an established obligation is due now, every swipe alternative must honor or materially address it while varying the delivery, consequence, perspective, or complication. An original or inferred idea can also become primary once its causal route is ready, but it remains an idea rather than a retroactive promise.
