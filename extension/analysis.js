...
export const EXTREME_CANON_INSTRUCTION = 'USER-ESTABLISHED CANON FIDELITY — Explicit user/OOC continuity assertions are authoritative even when statistically extreme, unprecedented, off-scale, unique, or beyond familiar setting records. Preserve their semantic magnitude, rank, scope, comparisons, and qualifiers exactly in canon_constraints and in any relevant guidance. Do not regress an outlier toward the mean, cap it at a franchise record, reinterpret it as rumor, downgrade “off the charts” or “among the highest in history” to merely high, or invent a conservative exact number. Setting averages and records provide contrast, not a ceiling. When the user establishes a relational extreme without an exact value, retain that relational constraint rather than fabricating precision; compatible outcomes include an exceeded instrument range, a result beyond the known record, or a need for verification. This fixes the fact, not every consequence: character reactions, causes, complications, and future outcomes remain open unless also established. A later explicit user/OOC correction may replace the constraint. Return canon_constraints as the complete current list of durable explicit user/OOC semantic constraints that must survive future context loss; preserve existing constraints until explicitly corrected.';
