...
const MODE_TREATMENT = Object.freeze({
    ...
    balanced: 'BALANCED TREATMENT: Give the NPC or world follow-through a clear, meaningful effect while preserving contested player outcomes.',
    fun: 'FUN TREATMENT: Give the NPC or world follow-through a prominent, lively expression without deciding the player’s contested response or result.',
});
...
export function formatBeatContract(_sceneValue, beatValue, options = {}) {
    ...
    return [
        'TALE FAIRY EXTERNAL-REACTION GUIDE: The user action is outside Tale Fairy’s authority, and so is the player’s resulting response. Resolve it only from the user text, established context, and the main roleplay instructions; resolve the response from those same authorities. Do not use this guide to infer, reinterpret, expand, narrow, relocate, complete, substitute, or judge the user action, its target, its manner, or the player’s intent, response, consent, or inner state.',
        'Tale Fairy begins only with what NPCs or the surrounding world do in response. It may add an external reaction, consequence, opportunity, or natural next causal step. NPCs and events may act toward, address, help, hinder, touch, threaten, or otherwise affect the player when the scene establishes that action or makes the effect unmistakable. For a contested action, present the external attempt while preserving the player’s meaningful chance to respond; do not decide an uncertain hit, defense, injury, restraint, compliance, or other player result. It may not deny, delay, weaken, cap, or otherwise modify the user action or an outcome explicitly established by the user.',
        'SELF-PROPELLING MOVEMENT: The selected branch must produce observable external development that exists independently of any player reply, including same-scene progress. Use completed NPC decisions, reactions, actions, disclosures, commitments, consequences, discoveries, opportunities, environmental or task progress. If travel or arrival is complete, start at the settled destination rather than repeating transit. In dialogue-centered scenes, let NPCs change the situation; never author the player character’s response, choice, consent, inner state, or an unsupported result.',
        'This is a one-response lease for the exact transcript it was planned from. Use it only for the next response when the latest user turn still matches the selected condition; after that response, discard it completely. If the latest turn changes subject, activity, or focus, ignore this guide rather than carrying its topic forward. Select exactly one closest-fitting branch for external forward motion and never combine branches. The extension has already selected the branch below with a weighted random choice. If every branch would author the player’s choice, response, consent, inner state, or an unsupported effect on them instead of only the NPC or world response, ignore them and let the main roleplay instructions govern the response.',
        ...
        MODE_TREATMENT[mode],
        'The selected next-step direction and effect govern only NPC or world follow-through. They never define the user action or decide the player’s response. They never control the player character; contested results remain open for the player to answer. They may establish an external action toward the player and an obvious resulting effect when the scene supports it.',
    ].join('\n');
}
