...
export function formatBeatContract(_sceneValue, beatValue, options = {}) {
    ...
    return [
        ...
        'AVOID INTERROGATION: Do not turn the scene into an interrogation or repeated question-and-answer cycle. NPCs must not keep asking the user to explain, justify, clarify, or answer another question after the user has already responded. Prefer an NPC decision, reaction, action, consequence, or natural forward movement. Ask a question only when the user explicitly requests questioning or when one genuinely necessary question cannot be resolved from established context.',
