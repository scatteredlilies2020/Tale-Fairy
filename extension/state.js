...
export function buildPromptPayload(state, { enabled = true, guidanceUsable = false, latestUserMessage = '' } = {}) {
    ...
    const latestUserOverride = `The latest user turn was written after this Tale Fairy guidance was planned and has higher priority. Every action, direct question, or choice the user declares is binding authorization for its ordinary continuation and immediate meaningful consequence; no words such as "advance" or "proceed" are required. Infer routine implied steps instead of making the user micromanage walking through a chosen doorway, reaching a stated nearby destination, receiving an available result, or letting an addressed character answer. Do not turn agency protection into a permission checkpoint, merely restate the action, insert a procedural obstacle without established cause, or end immediately before the obvious result. Stop for the user only when a genuinely new consequential choice is required and their existing action does not already decide it. If an action is impossible, show the attempt and concrete in-world obstacle. If any earlier Tale Fairy sentence conflicts with the latest user action or pacing, ignore that sentence.${explicitProgress ? ` The latest turn also explicitly commands forward progress. Complete its requested transition or reach its stated milestone in this reply; do not stop at another queue, doorway, preamble, permission question, or "about to" state. Any earlier Tale Fairy sentence that says not this turn, withhold the result, stop before the milestone, wait, or ask whether to proceed is void.` : ''}`;
