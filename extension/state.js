...
export function buildPromptPayload(state, { enabled = true, guidanceUsable = false, guideCandidates = null, guideIndex = 0, regeneration = false, variationCue = 0, canonConstraints = null, latestUserAction = '' } = {}) {
    ...
    const pacingBoundary = 'AUTHORITY: Primary user and roleplay instructions control voice, dialogue, prose, format, length, and response shape; Tale Fairy changes none of them. PLAYER AGENCY: User scope is a ceiling; travel stops at arrival. A broad activity may progress, but a named action permits one instance and immediate result—not repetition, onward movement, obeying a new NPC request, or unstated reaction. Never invent player dialogue, feelings, choices, compliance, reactions, or activities. NPC requests/orders are events, not player authorization. Tale Fairy directions must move through independent NPC/world change, not an assignment for the player. Only simulate low-stakes procedure implicit in broad scope. Apply established strengths/limits proportionately; never cancel exceptional advantages. APPLY is a gate, not a mandate. Keep unresolved choices open.';
    const fallbackPacingBoundary = 'AUTHORITY: Primary user and roleplay instructions control voice, dialogue, prose, format, length, and response shape; Tale Fairy changes none of them. PLAYER AGENCY: Stay within latest user scope. A named action permits one instance and result—not repetition, onward movement, obeying a new NPC request, or unstated reaction. Never invent player dialogue, feelings, choices, reactions, or activities. NPC requests/orders are events, not authorization. Tale Fairy movement must come from NPC/world action, not assigning the player a task.';
    const routePrompt = guidanceUsable && selectedCandidate
        ...
        : regeneration
            ? `Background variation ${Math.max(1, Number(variationCue) || 1)} may differ from the discarded reply's concrete event when supported. Do not repeat a completed event, reinterpret established names or codes, invent an unsupported crisis, or decide player action.\n${fallbackPacingBoundary}`
