...
import { defaultPlannerSchedule, markPlannerCompleted, normalizePlannerSchedule } from './planner-scheduler.js?v=0.14.19';
import { defaultCausalContext, defaultSceneProfile, formatCausalContext, hasUsableCausalContext, normalizeCausalContext, normalizeSceneProfile } from './causal-context.js?v=0.14.19';
...
import { TALE_FAIRY_CONTEXT_GUIDE, isStoryGeneration, refreshGameMasterContract } from './game-master.js?v=0.14.19';
import { relevantActors } from './evidence-selection.js?v=0.13.9';
import { defaultPreparedWorld, normalizePreparedWorld, preparedWorldForPrompt, formatPreparedWorld, formatPacingPreference } from './prepared-world.js?v=0.14.19';
...
function normalizeRequestVerification(value) {
    ...
    return {
        ...
        causalContext: normalizeCausalContext(value.causalContext),
...
export function normalizeState(input = {}) {
    ...
    const causalContextUpgrade = inputVersion > 0 && inputVersion < 57;
    ...
    const state = {
        ...
        lastRequestVerification: causalContextUpgrade ? null : normalizeRequestVerification(value.lastRequestVerification),
    };
    ...
    // Retire the old prescriptive branch payload completely rather than leaving
    // it in chat metadata where it can consume context or be mistaken for live
    // guidance by an older integration surface.
    delete state.beatDirective;
    delete state.beat_directive;
    return state;
}
...
export function stateForPrompt(state, { query = '' } = {}) {
    ...
    return {
        ...
        preparedWorld: preparedWorldForPrompt(s.preparedWorld, { query }),
