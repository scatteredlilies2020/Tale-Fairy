...
export function buildPromptPayload(state, { enabled = true, guidanceUsable = false, guideCandidates = null, guideIndex = 0, regeneration = false, variationCue = 0, canonConstraints = null, latestUserAction = '' } = {}) {
    ...
    const canon = boundedPromptLines(promptCanon, '- ', 300, 1700);
    const canonPrompt = canon
        ...
        : '';
    const notes = boundedPromptLines(s.userNotes.map(note => `${noteLabels[note.kind]}: ${note.text}`), '- ', 300, 1700);
    ...
    const pacingBoundary = 'PACING: User scope is a ceiling; travel ends at arrival. Broad bounded activities may progress; named moves stay narrow. Simulate routine micro-actions without prompting and show concrete progress. Apply established strengths/limitations proportionately; never cancel exceptional advantages. Never invent player dialogue, feelings, consequential choices, or new activities. APPLY is a gate, not an order to make it true. Linger on consequential, emotional, relational, revelatory, conflict, or discovery-rich beats; compress repetitive procedure and menus unless engaged. Do not routinely end with a question, choices, “your call,” or expectant waiting. Ask only when naturally necessary; otherwise end on concrete story movement while leaving the player free to act. Keep unresolved choices open; advance independent NPC/world action rather than choosing or prompting.';
    const fallbackPacingBoundary = 'PACING: Stay within latest user scope; never invent player dialogue, feelings, choices, or activities. Linger on engaged emotional, relational, revelatory, conflict, discovery, or consequential beats; compress low-yield procedure and menus. Do not routinely end by asking, offering choices, saying “your call,” or waiting expectantly. Ask only when naturally necessary; otherwise end on concrete story movement with player agency open. Keep choices open; advance independent world action.';
    const routePrompt = guidanceUsable && selectedCandidate
        ? `${conductorPrompt}Conditional authorial direction${regeneration ? ' for a different regeneration' : ''}:\n${authorialDirective(selectedCandidate)}\nWhen its APPLY condition holds and its exclusion does not, fulfill STORY FUNCTION within IMPACT ENVELOPE. This direction is binding at the level of narrative purpose, not a prescribed incident. Realize events from full context; never paraphrase notes or predetermine impact. If invalid, preserve the frame and choose the most coherent realization.${regeneration ? ' Do not reuse the discarded reply\'s concrete realization.' : ''} Keep private future developments offscreen and preserve established meanings, pacing, and player agency.\n${pacingBoundary}`
        : regeneration
