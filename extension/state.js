...
export function buildPromptPayload(state, { enabled = true, guidanceUsable = false, guideCandidates = null, guideIndex = 0, regeneration = false, variationCue = 0, canonConstraints = null, latestUserAction = '' } = {}) {
    ...
    const worldInitiative = 'REQUIRED WORLD INITIATIVE: Make one concrete, noticeable character/world/system/environment action or change happen in-scene—even in quiet routine—so the player has something new to perceive or react to. Ground it in the latest turn and full context. Do not substitute options, a player task, or a question; leave the player\'s response open.';
