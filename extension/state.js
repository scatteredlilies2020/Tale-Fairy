...
export function buildPromptPayload(state, { enabled = true, guidanceUsable = false, latestUserMessage = '' } = {}) {
    ...
    const latestUserOverride = explicitProgress
        ? `The latest user turn explicitly commands forward progress. Treat that as binding minimum progress. Complete the requested transition or reach its stated milestone in this reply; do not stop at another queue, doorway, preamble, permission question, or "about to" state. Any earlier Tale Fairy sentence that says not this turn, withhold the result, stop before the milestone, wait, or ask whether to proceed is void. If established facts make the requested result impossible, show the attempt and concrete obstacle instead of stalling.`
        : `The latest user turn was written after this Tale Fairy guidance was planned and has higher priority. Carry out the natural immediate consequence of any action the user already declared. Do not turn agency protection into a permission checkpoint, repeat the action without advancing it, or end immediately before its obvious result. If any earlier Tale Fairy sentence conflicts with the latest user action or pacing, ignore that sentence.`;
