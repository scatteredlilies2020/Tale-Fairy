...
export function buildPromptPayload(state, { enabled = true, guidanceUsable = false, guideCandidates = null, guideIndex = 0, regeneration = false, variationCue = 0, canonConstraints = null, latestUserAction = '' } = {}) {
    ...
    const worldInitiative = 'REQUIRED WORLD INITIATIVE: Make one noticeable NPC/world/environment change in-scene, even in routine, for the player to perceive or react to. Ground it in current context; when relevant, let the durable trajectory shape it so long-range influence accumulates. Do not substitute options, a player task, or a question; leave response open.';
