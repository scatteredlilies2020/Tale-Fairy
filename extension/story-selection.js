...
import { ownedInput, ownedPass, OWNED_SCHEMA, needsEventReframe } from './event-planning.js?v=0.14.34';
...
import { SELECTED_MATERIAL_SCHEMA, validateSelectedMaterial } from './selected-material.js?v=0.14.35';
...
Prepare open story substance for this RP or simulation in one JSON response, not a prewritten scene. Match its premise and causality; ordinary relationships and nonhuman processes are as valid as adventure. No mandatory conflict, escalation, travel or ending. Supply no writing-style, narration, tone, tempo or dramatic instructions. Only the available circumstances affect pacing; the writer supplies execution and incidental detail.
...
available names optional opportunities and their relevant conditions, NOT a recap of tasks or accomplishments; developing names substantive activities or processes these could support; lasting names the open longer-term possibilities. Each horizon adds substance, not just "this interest can continue" or a longer timescale. All three concern the discoverable surface and its possible reach, never secret actors, motives or causes. Access to an NPC does not reveal their unspoken history or private expertise: omit those details even from lasting. Keep unestablished dependencies conditional. subjectIds names all contributors and is not writer-facing.

USEFUL CREATIVE CONTRIBUTION
Distinguish current changes, unresolved matters and forecasts. Do not recycle provocation/counter/observation under new wording. Use creative license for unrealized possibilities grounded in established abilities, interests, resources and commitments, not just summary or prediction. Give an activity actual subject matter: what could be made, practiced, learned, shared or explored, and what makes it interesting here. "Refine the work", "develop rapport", "use their strengths" and "build a reputation" are headings, not substance. Player competence enables participation, not perpetual testing. Quiet enjoyment counts. Preserve unresolved conflict without making it the only activity. No forced reconciliation or novelty quota; select nothing when nothing supported adds value.

Several private aims can inform ONE circumstance: combine their subjectIds, not separate owner paragraphs or ordered activities. Select worthwhile experiences, not prerequisites or more approvals. Completion need not become another task, scrutiny or dilemma. No scripted success/failure branches.

Remove spent portions: arrival ends the journey, not its wider undertaking. Opportunities need not interrupt current activity. Leave choices, responses and outcomes open. Never claim unseen actions, offers or agreements happened. No arbitrary restrictions, resolved pending business, dialogue or ordered beats. Specific proposed content is welcome; new places, possessions and obligations are not established by ambiguity. Proposals are not canon.
