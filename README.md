...
- Prepares a conditional direction set one turn ahead: one primary movement, two materially distinct redirect-safe alternatives, and an explicit use-none rule. The roleplay model selects exactly one fitting branch after reading the newly appended user action. If none fits, Tale Fairy contributes nothing; a used or older set is never revived as a fallback.
