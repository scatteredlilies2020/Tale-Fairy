...
- Keeps pacing under direct user authority: the latest user/OOC turn sets the maximum time, activity, and player progress a response may cover. A moment or named action cannot silently become repeated action, a scene transition, or an additional player choice; user-authorized travel may arrive but does not perform the next activity at the destination.
