...
- Uses context-conscious defaults: 12 recent messages at 700 characters each, a 12,000-character soft planner payload, compact state collections, and a 2,200-token planner response ceiling. Hidden thinking/stat blocks are removed from planner excerpts, while the latest turn receives extra room. Optional context is compacted first while relevant current user/OOC material is retained.
