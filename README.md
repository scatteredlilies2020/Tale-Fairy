...
- Prepares a total-coverage conditional direction set one turn ahead: one primary movement and two materially distinct redirect-safe alternatives. After reading the newly appended user action, the roleplay model always selects exactly one closest-fitting branch and never combines them. A fresh successful plan therefore always adds playable movement; quiet listening, assignments, rest, and other routine scenes receive small context-native development rather than forced conflict or interruption. If planning is unavailable or stale, Tale Fairy safely contributes nothing, and a used or older set is never revived as a fallback.
