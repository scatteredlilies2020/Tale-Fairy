...
Since **0.13.18**, every tier honors Tale Fairy's **Reasoning / thinking** setting; routine updates and bounded reviews no longer force Off. **Auto** inherits the selected profile or its preset, then the active SillyTavern effort, falling back to provider defaults when neither specifies one. Explicit levels apply to direct, profile, and active-model requests, including recovered-output repairs. More reasoning can take longer and, where the provider counts it against the response limit, leave less room for the final JSON. The compact input tiers and retry caching remain enabled. If a provider rejects disabled reasoning, the existing single compatibility retry also clears nested custom-provider disable controls; it does not keep retrying indefinitely. No model-specific setting is silently changed for speed.
