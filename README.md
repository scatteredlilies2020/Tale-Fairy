...
- Reuses the exact completed pre-reply plot context for both Regenerate and swipe. A fallback may pick up an already-ready matching plan without a new call. A rolling history of 12 input snapshots survives reloads and supports deleting back to a matching input. Discarded prose never becomes canon.
...
For chats stranded by the older cache bug, reopening with no usable saved pre-reply plan starts **one background repair evaluation per input**, not a Full Rebuild or an evaluation per swipe. The attempt is recorded across reloads; a failed attempt can be retried explicitly with **Guide now**. Generation never waits, and rapid Regenerate/swipe requests do not cancel an active repair for that same source. Guide now also targets the pre-reply source after a retry. Repairs exclude the discarded reply and potentially newer global/Continuity summaries; accepted transcript recaps and lore remain available. Existing bounded validation/connection retries still apply to the single evaluation. A completed repair becomes available for the next matching request without changing a request already sent.
