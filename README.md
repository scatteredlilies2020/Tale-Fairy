...
- Privately archives and evaluates the full route set, but sends the roleplay model only a compact layered authorial frame plus one conditional narrative function and impact envelope. Alternatives stay inside Tale Fairy. Every roleplay request requires one visible, context-supported character/world initiative while leaving the player's response open. Regenerates and swipes rotate to distinct functions, reuse only pre-response canon, and reject replaying the discarded realization; if no aligned archive exists, a compact active fallback derives the initiative from the latest turn instead of stale planning.
