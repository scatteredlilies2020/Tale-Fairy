...
- Privately archives and evaluates the full route set, but sends the roleplay model only a lean director cue: the selected immediate direction and its intended shift. Conditions, evidence, strength labels, execution notes, alternatives, and compliance checks stay inside Tale Fairy. Regenerates and swipes rotate the cue for materially different developments while retaining continuity; if no aligned archive exists, a compact generic motion/variety fallback is still injected.
