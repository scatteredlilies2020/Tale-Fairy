...
- Uses context-conscious defaults: 12 recent messages at 700 characters each, a 12,000-character soft planner payload, and compact state collections. The 4,096-token response ceiling leaves reasoning models enough room for the complete structured plan; normal concise outputs stop well before it. Hidden thinking/stat blocks are removed from planner excerpts, while the latest turn receives extra room. Optional context is compacted first while relevant current user/OOC material is retained.
