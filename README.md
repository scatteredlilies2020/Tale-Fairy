...
- Injects an adaptive forward-motion block into every normal roleplay request. The latest user action and its ordinary intended result always come first; Tale Fairy contributes a compatible response, consequence, opportunity, or next causal step afterward.
...
- Does not control pacing or cap outcomes. Explicit and naturally implied user intent binds, including the ordinary target and minimal positioning implied by terse actions. Tale Fairy never substitutes a literal action with a metaphorical or interface-based analogue, and never supplies the player character's dialogue, thoughts, feelings, consent, decisions, compliance, or unrelated extra action.
- Sends a general, observable next-step effect after the user action. It is subordinate follow-through, not an outcome restriction: if it conflicts with the user's intent or the natural next causal step, the writing model ignores it. All scale classifications, targets, evidence, and preserve/forbid constraints remain private planner context.
...
- Prepares a conditional follow-through set one turn ahead: one primary movement and two materially distinct redirect-safe alternatives. After reading the newly appended user action, the roleplay model performs that action and its immediate causal result first, then selects one compatible branch. It ignores every branch if they would redirect, delay, narrow, weaken, or cap the action. Quiet listening, assignments, rest, and other routine scenes still receive context-native movement rather than forced conflict or interruption. If planning is unavailable or stale, Tale Fairy safely contributes nothing, and a used or older set is never revived as a fallback.
