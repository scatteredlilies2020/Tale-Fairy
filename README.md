...
- Replans in the background after each completed assistant response. The next roleplay request never waits for the planner and never makes an extra AI call. The bundled server plugin owns the model request, so planning continues if Android freezes or discards the page, the browser closes, the tab reloads, or another page opens; Tale Fairy reconnects and saves the retained result when that chat is available again. An unfinished pass may complete after one appended user turn. Transient planner or network failures receive at most two automatic retries; timeouts stop rather than silently starting another multi-minute request.
...
- Gives the planner its own adjustable 0–2 temperature with a synchronized slider and typed value. It applies to every planner path without changing the roleplay model's temperature; provider limits are enforced on the outgoing request (for example, GLM is capped at Z.AI's maximum of 1). A fresh variation nonce is included as normal prompt text; no provider seed parameter is required.
