...
- Prepares a conditional external-reaction set one turn ahead: one primary movement and two materially distinct redirect-safe alternatives. After reading the newly appended user text, the roleplay model resolves the action without Tale Fairy, then selects one compatible NPC/world branch. It ignores every branch that would interpret, redirect, delay, narrow, weaken, or cap the action. Quiet listening, assignments, rest, and other routine scenes still receive context-native external movement rather than forced conflict or interruption. If planning is unavailable or stale, Tale Fairy safely contributes nothing, and a used or older set is never revived as a fallback.
