...
- Converts the planner's exact scene-specific required effect, target, preserve/forbid constraints, and scale bounds into the provider prompt while leaving exact prose and concrete realization to the main roleplay model.
...
- Reuses the exact analyzed effect and constraints for regenerations/swipes while asking for a genuinely different realization. The planner sample remains archived for audit but is not reapplied as a generic provider overlay. Discarded prose never becomes canon.
